...
export default function OnboardingPlayerClient() {
  ...
  const [ctaText, setCtaText] = useState<string | undefined>(undefined)

  useEffect(() => {
    if (typeof window === "undefined") return
    try {
      const sp = new URLSearchParams(window.location.search)
      setCtaText(sp.get("cta") || undefined)
    } catch (e) {
      // ignore
    }
  }, [])
        ...
        {tab === "register" && (
          <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
            ...
            <GameButton data-testid="onboarding-register-submit" type="submit" className="mt-2" disabled={loading}>
              {loading ? "Loading..." : "Register as Player"}
            </GameButton>
          </form>
        )}
