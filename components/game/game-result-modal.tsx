      ...
      <div className="bg-green-100 border-4 border-green-700 rounded-2xl p-6 sm:p-8 w-[95vw] max-w-xl relative flex flex-col items-center shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full justify-center mt-2">
          ...
          {isOnlineMode && onRematch && (
            <>
              {rematchInviteId ? (
                // If I have an invite, show Accept/Decline
                <div className="flex gap-2 w-full sm:w-auto">
                  <GameButton
                    onClick={onAcceptRematch}
                    isLoading={rematchLoading}
                    className="w-full sm:w-32 py-3 bg-blue-600 hover:bg-blue-700 border-blue-800"
                  >
                    Accept
                  </GameButton>
                  <GameButton
                    onClick={onDeclineRematch}
                    className="w-full sm:w-32 py-3 bg-red-600 hover:bg-red-700 border-red-800"
                  >
                    Decline
                  </GameButton>
                </div>
              ) : (
                // Otherwise allow requesting
                <GameButton
                  onClick={onRematch}
                  isLoading={rematchLoading}
                  className="w-full sm:w-56 text-sm sm:text-base py-3 bg-purple-600 hover:bg-purple-700 border-purple-800"
                >
                  Rematch
                </GameButton>
              )}
          ...
          {/* Only show standard "Play Again" if NOT in online mode (where Rematch is used instead) */}
          {(!isOnlineMode || !onRematch) && (
