...
describe('Matchroom API (mock-first)', () => {
  ...
  test('create room, join, leave, start match', async () => {
    ...
    const createPath = paths.find((p: string) => /matchroom.*create|room.*create|\/api\/v1\/matchroom/.test(p))
  if (!createPath) { console.warn('Matchroom create missing - skipping'); report.record(moduleName,'create','pending'); return }

    const payload = { userId: host.user.id, gameType: "TicTacToe" }
    ...
    // player join
    const joinPath = paths.find((p: string) => /join.*room|room.*join|\/api\/v1\/matchroom.*join/.test(p))
  if (!joinPath) { console.warn('Matchroom join missing - skipping join tests'); report.record(moduleName,'join','pending'); return }
    ...
    const joinPayload = { matchCode: roomCode, user: player.user.id }
    const rJoin = await callApi({ method: 'post', path: pathWithId, body: joinPayload, headers: { authorization: `Bearer ${player.token}` } })
    ...
    // start match (host)
    const startPath = paths.find((p: string) => /start.*match|match.*start/.test(p))
